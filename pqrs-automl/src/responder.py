...
src/responder.py
----------------
Genera respuestas automáticas a PQRS usando Groq API (LLaMA 3)
con plantillas de respaldo si la API no está disponible.
"""
...
# ─────────────────────────────────────────────
# Plantillas de respaldo por categoría y urgencia
# ─────────────────────────────────────────────
TEMPLATES = {
    "Petición": {
        "Alta": [
    ...
    "Reclamo": {
        "Alta": [
            "Su reclamo ha sido recibido y marcado con carácter urgente. De acuerdo con la normativa vigente, tiene derecho a una respuesta formal en un plazo máximo de 15 días hábiles. Nuestro equipo jurídico revisará su caso de inmediato.",
...
def generate_response(
    ...
    Intenta Groq primero, cae a plantilla si falla.
    Returns: (respuesta, fuente)
    """
    if api_key:
...
def get_api_key() -> str:
    """Obtiene la API key de Groq desde Streamlit secrets o variable de entorno."""
    try:
